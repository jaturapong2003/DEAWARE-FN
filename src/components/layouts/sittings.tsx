...
import { SettingsIcon, ImageIcon, UploadCloud } from 'lucide-react';
...
const SettingsDialog: React.FC = () => {
  ...
  const updateProfileMutation = useMutation<{ message?: string }>({
    ...
    onError: (error: Error) =>
      toast.error(error?.message || 'อัปเดตข้อมูลโปรไฟล์ล้มเหลว'),
  });
  ...
  const uploadProfileImagesMutation = useMutation<{ message?: string }>({
    mutationFn: async (): Promise<{ message?: string }> => {
      ...
      const url = baseUrl.endsWith('/api')
        ? baseUrl.replace('/api', '') + '/api/employee/profile-image'
    ...
    onError: (error: Error) =>
      toast.error(error?.message || 'อัปโหลดรูปโปรไฟล์ล้มเหลว'),
  });
  ...
  const uploadFaceImagesMutation = useMutation<{ message?: string }>({
    ...
    onError: (error: Error) =>
      toast.error(error?.message || 'อัปโหลดรูปใบหน้าล้มเหลว'),
  });
      ...
      <DialogContent>
        ...
        <Tabs defaultValue="UploadProfileImages">
          <TabsList variant="line">
            <TabsTrigger value="UploadProfileImages">
              อัพโหลดรูปโปรไฟล์
            </TabsTrigger>
            <TabsTrigger value="UploadFaceImages">อัพโหลดรูปใบหน้า</TabsTrigger>
            <TabsTrigger value="UpdateProfileData">
              อัปเดตข้อมูลโปรไฟล์
            </TabsTrigger>
          </TabsList>
          <TabsContent value="UploadProfileImages">
                        ...
                        {profileFiles.length > 0 ? (
                          <>
                            <div className="flex -space-x-2">
                              {[...Array(Math.min(profileFiles.length, 3))].map(
                                (_, i) => (
                                  <div
                                    key={i}
            ...
            <DialogFooter>
              <Button
                type="button"
                disabled={
                  uploadProfileImagesMutation.isPending ||
                  profileFiles.length === 0
                }
          ...
          <TabsContent value="UploadFaceImages">
                    ...
                    <div className="group border-border/40 bg-secondary/10 hover:border-primary/40 hover:bg-secondary/20 relative overflow-hidden rounded-2xl border-2 border-dashed transition-all duration-300 ease-in-out">
                      ...
                      <div className="relative z-10 flex h-24 flex-col items-center justify-center gap-2 transition-transform duration-300 group-active:scale-95">
                        {faceFiles.length > 0 ? (
                          <>
                            <div className="flex -space-x-2">
                              {[...Array(Math.min(faceFiles.length, 3))].map(
                                (_, i) => (
                                  <div
                                    key={i}
                                    className="border-background bg-primary text-primary-foreground flex h-8 w-8 items-center justify-center rounded-full border-2 text-[10px] font-bold shadow-sm"
            ...
            <DialogFooter>
              <Button
                type="button"
                disabled={
                  uploadFaceImagesMutation.isPending || faceFiles.length === 0
                }
                onClick={() => uploadFaceImagesMutation.mutate()}
              >
                {uploadFaceImagesMutation.isPending
                  ? 'กำลังอัปโหลด...'
                  : 'อัปโหลด'}
              </Button>
            </DialogFooter>
          ...
          <TabsContent value="UpdateProfileData">
            <div className="grid gap-4 py-4">
              <FieldGroup>
                <Field>
                  <FieldLabel htmlFor="profile-phonenumber">
                    เบอร์โทรศัพท์
                  </FieldLabel>
                  <Input
                    ...
                    type="text"
                    placeholder={
                      user?.phone_number || 'กรอกเบอร์โทรศัพท์ของคุณ'
                    }
                    value={phoneNumber}
